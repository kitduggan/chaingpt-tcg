...
export default function PackModal({ open, packIndex, packOrigin, onClose }: PackModalProps) {
  ...
  const handlePackClick = () => {
    ...
    // Save cards to localStorage — no wallet popup, completely free
    if (address) {
      addEarnedCards(address, result.cards.map(c => c.id))
    }
      ...
      {open && (
        <>
          ...
          <div className="fixed inset-0 z-[201] flex items-center justify-center" onClick={handleClose}>
            ...
            {/* Close hint / connect nudge */}
            <AnimatePresence>
              {stage === 'revealed' && (
                <motion.div
                  className="fixed bottom-10 left-0 right-0 flex flex-col items-center gap-1 pointer-events-none"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  transition={{ delay: 0.5 }}
                >
                  {!address && (
                    <p className="font-pixel text-[7px] text-[#00f5ff]/70">
